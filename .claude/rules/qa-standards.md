...
## 铁律：独立 agent code review 始终必做（C-HARNESS-03）

**每个涉及代码改动的迭代，Layer 3（独立 agent code review）不可跳过、不可由作者自审替代。**

- PLAN 拆解里必须把"独立 code review"列成一个**显式阶段/任务**，不是埋进真机准出的附属步骤。
- 执行该 review 的 agent 必须 ≠ 实现改动的 agent / 主循环（消除 author-bias）。
- 高风险改动（消息热路径 / 破坏性群管 / 刷框架模块 / 反检测配置）在此之上再加 Layer 4 Santa 双独立审。
- 输出结构化 PASS/FAIL；有 FAIL → Fix Cycle，复审通过才算 done。
- **唯一可降级**：纯文档/注释/只读诊断脚本——降级须在 REVIEW 留一行记录（降级决策可追溯）。
