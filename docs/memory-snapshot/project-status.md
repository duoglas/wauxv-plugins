...
- **下一步 P2-b(cutover, 现在只设计不执行)+ P3**:cutover = config union 迁移 + DB 接入 + 停旧 RP 插件, **合并插件落新建 `GroupAdminPlus/`**(用户定, 以备回切), 留 P3 完成后另开 PLAN+Santa 双审执行(真机不可逆)。P3 = 伸手党 freeloader→doWarn 直连 + 设备侧(检测/worker/反射领取者/UI/Job调度/快照 P+G 延迟拍照/G夹取/孤儿快照#24)接线 + 端到端真机。
