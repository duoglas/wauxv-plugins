...
// 红包运行态 (内存 Map, nativeurl -> Object[]{talker, sender, msgid, attempt})。生命周期短, 不落盘。
java.util.Map RP_STATE = java.util.Collections.synchronizedMap(new java.util.HashMap());
...
// 当前正在处理的 nativeurl (单红包串行: 同一时刻只跟一个红包的页面流转)。
String CUR_NATIVEURL = null;
...
void onLoad() {
    try { log("RedPacketStats v1.0.7 loaded."); } catch (Throwable t) {}
    ...
    try {
        ...
        hbBgLog("[RP] " + hbNow() + " v1.0.7 loaded. per-group enable (default OFF), config via onClickSendBtn (NOT to group), dialog, "
            + "detect→[skip exclusive]→delay(jittered)→open cover→[手气→detail|查看领取详情→skip normal cover|no-entry→retry(2x,jittered)]→reflect(jittered)→[skip normal/equal]→exclude-filter→rate-limit→tier(random variant)→one @msg. "
            + "v1.0.7: per-group exclude list(@add/del+dialog), jitter(去零方差), at_limit=10, same-group rate-limit, detect-fields removed, finish精确(luckymoney only). 措辞随机=pickConnector(金额连接词)+pickActionPhrase(动作语气包裹); 动作核心/档位/前缀原样不随机. ANR rules enforced.");
    } catch (Throwable t) {
...
void onHandleMsg(Object msg) {
    try {
        ...
        try {
            if (content != null && sender != null && sender.equals(getLoginWxid()) && content.indexOf("红包排除") >= 0) {
        ...
        // 存运行态
        RP_STATE.put(fKey, new Object[]{ fTalker, fSender, Long.valueOf(fMsgid), Integer.valueOf(0) });
        ...
        hbBgLog("[RP] " + hbNow() + " redpacket detected in enabled group (type=" + type
            + ", hasNativeurl=" + (nativeurl != null) + "). scheduling first hbProcess in " + (delayMs / 1000L) + "s (cfg=" + delaySec + "s, jittered, attempt=0).");

        delay((int) delayMs, new Runnable() {
            public void run() {
                try { hbProcess(fKey); }
                catch (Throwable t) { hbBgLog("[RP] " + hbNow() + " scheduled hbProcess err: " + t); }
            }
        });
    } catch (Throwable t) {
...
// ============ hbProcess: 开封面页 (一次尝试) ============
// ================================================================
void hbProcess(String key) {
    try {
        Object[] st = (Object[]) RP_STATE.get(key);
        if (st == null) { hbBgLog("[RP] " + hbNow() + " hbProcess: state gone for key, skip."); return; }
        int attempt = ((Integer) st[3]).intValue();
        String talker = (String) st[0];
        CUR_NATIVEURL = key;
        ...
        if (hostCtx == null) { hbBgLog("[RP] " + hbNow() + " ABORT: no hostContext."); return; }
        ...
        hbBgLog("[RP] " + hbNow() + " hbProcess err: " + t);
    }
}
...
void hbOnActivityResume(Object thisObject) {
    try {
        ...
        // 只处理 bot 自己正在跟的红包 (CUR_NATIVEURL 非空说明刚开了页)
        if (CUR_NATIVEURL == null) return;
        ...
        if (isReceive) {
            COVER_ACT.put(CUR_NATIVEURL, idHash);
        ...
        // NewDetailUI: 后台反射读明细
        final String fKey = CUR_NATIVEURL;
...
void afterCoverNormalSkip() {
    final String key = CUR_NATIVEURL;
    finishActivityByHash(COVER_ACT.get(key));
    COVER_ACT.remove(key);
    CUR_NATIVEURL = null;
    if (key != null) RP_STATE.remove(key);
    SEEN.clear(); // 本红包流转结束, 清 Activity 去重防内存增长
    hbBgLog("[RP] " + hbNow() + " skip normal: cover finished, state cleared, no detail/no msg.");
}

// 没领完: finish 封面 + 按 attempt 重试或放弃。
void afterCoverNoDetail() {
    ...
    if (attempt >= 2) {
        ...
        hbBgLog("[RP] " + hbNow() + " GIVE UP: attempt=" + attempt + " still not finished. drop this redpacket.");
        return;
    }
    ...
    st[3] = Integer.valueOf(nextAttempt);
    RP_STATE.put(key, st);
    SEEN.clear(); // 清 Activity 去重, 下次重开封面是新实例

    hbBgLog("[RP] " + hbNow() + " not finished at attempt=" + attempt + ". retry in " + (retryMs / 1000L) + "s (cfg=" + retrySec + "s, jittered, attempt=" + nextAttempt + ").");
    final String fKey = key;
    delay((int) retryMs, new Runnable() {
        public void run() {
            try { hbProcess(fKey); }
            catch (Throwable t) { hbBgLog("[RP] " + hbNow() + " retry hbProcess err: " + t); }
        }
    });
}
...
void hbDetailExtract(Object root, String key) {
    try {
        if (key == null) { hbBgLog("[RP] " + hbNow() + " detail: key null, skip."); return; }
        if (DONE.contains(key)) { hbBgLog("[RP] " + hbNow() + " detail: already done, skip."); return; }

        Object[] st = (Object[]) RP_STATE.get(key);
        String talker = (st != null) ? (String) st[0] : null;

        // BFS 找领取者 List
        Object listObj = hbFindReceiverList(root);
        if (listObj == null) {
            hbBgLog("[RP] " + hbNow() + " detail: no receiver list found (reflect).");
        ...
        try {
            ...
            if (rn >= 2) {
                ...
                if (allEqual) {
                    hbBgLog("[RP] " + hbNow() + " skip normal(equal amounts): " + rn + " receivers all equal. no message sent.");
                    // 清状态、关页, 不发任何消息。
                    finishDetailAndCover(key);
                    DONE.add(key);
                    RP_STATE.remove(key);
        ...
        // 分档 + 发一条 @ 消息
        if (talker != null) hbTierAndSend(talker, receivers);
        ...
        hbBgLog("[RP] " + hbNow() + " detail extract err: " + t);
...
// ============ 分档 + 发一条 @ 消息 (SPEC §6) ============
void hbTierAndSend(String talker, List receivers) {
    try {
        ...
        // 达标总人数 > rp_at_limit → 发一条无 @ 通用群规消息 (SPEC §6.1; v1.0.7 §14: 前缀+动作核心+档位=原值不动, 仅连接词+语气包裹随机)
        if (totalQualified > atLimit) {
            String rule = cfgRulePrefix() + "，" + pickConnector(t1) + pickActionPhrase(cfgTxt1())
                + "，" + pickConnector(t2) + pickActionPhrase(cfgTxt2())
                + "，" + pickConnector(t3) + pickActionPhrase(cfgTxt3());
            sent = rule;
            sendText(talker, rule);
            hbBgLog("[RP] " + hbNow() + " tier: qualified=" + totalQualified + " > at_limit=" + atLimit
                + " (excluded=" + excludedHit + ") -> sent generic group-rule msg (NO @, random connector+action-phrase, action-core/prefix/tier verbatim). counts: t1=" + g1.size() + " t2=" + g2.size() + " t3=" + g3.size() + ".");
        } else {
            // ≤ rp_at_limit → 现有逻辑: 一条消息, 按档分组, [AtWx=] 单独 @ (v1.0.7 §14: 动作核心+档位原值不动, 仅连接词+语气包裹随机)
            StringBuilder sb = new StringBuilder();
            boolean first = true;
            ...
            sent = sb.toString();
            sendText(talker, sent);
            hbBgLog("[RP] " + hbNow() + " tier: qualified=" + totalQualified + " <= at_limit=" + atLimit
                + " (excluded=" + excludedHit + ") -> sent @msg (random connector+action-phrase, action-core/tier verbatim). counts: t1=" + g1.size() + " t2=" + g2.size() + " t3=" + g3.size() + ".");
        }

        // v1.0.6 §13.3: 发送成功后更新本群上次发送时间 (限频)。
        if (sent != null && talker != null) RP_LAST_MSG.put(talker, Long.valueOf(System.currentTimeMillis()));
    } catch (Throwable t) {
