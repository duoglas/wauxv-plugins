...
### 4.4 警告带理由 + 自动来源静默（v1.19.0）

- **解析（@ 路径）**：`@TA 警告 <理由>` —— 当消息倒数第二 token 为"警告"且其本身非动作词（即末尾还跟着一个理由 token）时，识别为带理由警告：动作词改写为"警告"，把末尾 token 作为 `warnReason` 透传。纯字符串/内存判定，仅在末尾邻接"警告"时触发，不进普通消息热路径（C-PERF-01）；与"管理员 N"天数解析并列、互不影响。
- **doWarn 签名**：保留旧 3 参 `doWarn(g,s,t)`（转调 `doWarn(g,s,t,null)`），新增 4 参 `doWarn(g,s,t,reason)`。`auto = reason!=null && !reason.trim().isEmpty()` 判定是否"自动来源（带理由）"。
- **通知拼理由**：理由非空时，三条通知文案（`n>=kick` 自动移除 / `n==1` / 其余）**末尾**各追加 ` · <理由>`（`why`）。计数 / `wnt_` / `wnf_` / 踢人 / 清零逻辑逐字不变。
- **自动来源对豁免对象静默跳过**：当 `auto==true` 时，`canActOn` 失败（权限不足/不能操作该对象，含受保护/原生群主/越权）与 `isWhitelistAny`（白名单）命中两种豁免分支**静默 return**，不发"❌ 权限不足…"/"🛡 …在白名单"提示。**手动警告（reason 为空，如 `@TA 警告` 或引用回复触发）行为完全不变**，仍发对应提示。
- **不带理由的"警告"**：仍走 `dispatchAction` 原路径调旧 3 参 `doWarn`（=4 参 null），行为不变；不影响踢/请/管理员等既有动作及 @所有人守卫。
- **自动来源额外豁免管理员/群主/原生群主（防误踢管理，v1.19.1）**：当 `auto==true` 时，在 `canActOn` 判断之前先豁免 `isAdminInGroup` / `isOwner` / `isNativeOwner` 命中者并静默 return —— `canActOn` 对 owner→普通管理员不挡，潜水管理抢红包若被自动伸手党流程警告会累计→被机器人踢出，故须显式豁免。手动警告（`auto==false`）不受影响，仍可警告管理员。
